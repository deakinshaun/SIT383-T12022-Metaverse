...
float3 ovrGetAttributeTexCoord(int attributeRowOffset, uint vertIndex, int numAttributes, float slice) {
...
float4 OvrGetVertexPositionFromTexture(uint vid, int numAttributes, bool applyOffsetAndBias, float slice) {
  float4 pos = tex3Dlod(u_AttributeTexture, float4(ovrGetPositionTexCoord(vid, numAttributes, slice), 0));
  [branch]
...
float4 OvrGetVertexNormalFromTexture(uint vid, int numAttributes, bool applyOffsetAndBias, float slice) {
  float4 norm = tex3Dlod(u_AttributeTexture, float4(ovrGetNormalTexCoord(vid, numAttributes, slice), 0));
  [branch]
  if (applyOffsetAndBias) {
...
float4 OvrGetVertexTangentFromTexture(uint vid, int numAttributes, bool applyOffsetAndBias, float slice) {
  float4 tan = tex3Dlod(u_AttributeTexture, float4(ovrGetTangentTexCoord(vid, numAttributes, slice), 0));
  [branch]
  if (applyOffsetAndBias) {
