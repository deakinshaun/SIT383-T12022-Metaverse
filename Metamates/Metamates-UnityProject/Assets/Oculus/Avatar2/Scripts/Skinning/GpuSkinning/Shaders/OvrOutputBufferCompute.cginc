...
uint CalculatePositionOutputIndex(uint vertex_output_index)
{
  return vertex_output_index;
}

uint CalculateDoubleBufferedPositionOutputIndex(uint vertex_output_index, bool write_to_second_slice)
{
  // * 2 due to double buffering, then maybe +1 if writing to second slice
  return 2u * vertex_output_index + (write_to_second_slice ? 1u : 0u);
}

uint CalculateNormalOutputIndex(uint vertex_output_index, bool has_tangents)
{
  // *2 if interleaving tangent
  return (has_tangents ? 2u : 1u) * vertex_output_index;
}

uint CalculateTangentOutputIndex(uint vertex_output_index)
{
  // * 2 due to interleaved normals + tangent, then +1 for the tangent
  return 2u * vertex_output_index + 1u;
}

uint CalculateDoubleBufferedNormalOutputIndex(uint vertex_output_index, bool has_tangents, bool write_to_second_slice)
{
  // * 2 due to double buffering, then an additional *2 if interleaving tangent, then maybe +1 if writing to second slice
  return (has_tangents ? 4u : 2u) * vertex_output_index + (write_to_second_slice ? 1u : 0u);
}

uint CalculateDoubleBufferedTangentOutputIndex(uint vertex_output_index, bool write_to_second_slice)
{
  // * 4 due to double buffering and interleaved normals + tangent, then maybe +3 if writing to second slice
  return 4u * vertex_output_index + (write_to_second_slice ? 3u : 2u);
}
  ...
  bool write_to_second_slice)
{
  ...
  const uint norm_output_index = double_buffer ?
